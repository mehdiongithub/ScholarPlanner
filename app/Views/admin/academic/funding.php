...
<div class="location-nav">
    ...
    <a href="/admin/academic/funding" class="location-nav-link active">Funding Types</a>
</div>
...
<div class="location-layout">
    ...
    <div>
        <div class="form-card">
            <h2 style="font-size: 1.125rem; font-weight: 700; color: #1e293b; margin-top: 0; margin-bottom: 16px;">Add Funding Type</h2>
            <form action="/admin/academic/funding" method="POST">
                ...
                <table class="employees-table">
                    ...
                    <tbody>
                        <?php if (empty($fundings)): ?>
                            <tr>
                                <td colspan="3" style="text-align: center; color: #94a3b8; padding: 24px;">No funding types registered.</td>
                            </tr>
                        <?php else: ?>
                            <?php foreach ($fundings as $f): ?>
                                <tr>
                                    ...
                                    <td>
                                        <span class="status-badge <?= $f['status'] === 'active' ? 'active' : 'suspended' ?>"><?= e($f['status']) ?></span>
                                    </td>
                                    <td>
                                        <a href="/admin/academic/funding/<?= $f['id'] ?>/edit" class="action-link">Edit</a>
                                        <form action="/admin/academic/funding/<?= $f['id'] ?>/delete" method="POST" onsubmit="return confirm('Delete this funding type?');" style="display: inline;">
                                            <input type="hidden" name="csrf_token" value="<?= Security::csrfToken() ?>">
